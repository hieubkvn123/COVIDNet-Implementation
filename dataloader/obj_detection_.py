...
from augmentation_obj import augment_rotate
...
def batch_dataset(dataset,
    ...
    # dataset = dataset.map(lambda img, box, label: tf.py_function(augment_rotate, [img, box, label], [tf.float32, tf.float64, tf.int64]))
...
def disk_image_batch_dataset(directory,
    ...
    # @tf.function
    def parse_fn(path, bbox, label, augment=False):
